...
/**
 * 各板块共用「最早航空航天对照」过程模板：
 * A 中美阶段路线 → B 对比色块 → C 趋势曲线
 * （硬指标条与里程碑由 DomainPage 在其后接上）
 */
export function DomainProcess({ data }: { data: ProcessData }) {
  return (
    <div className="space-y-8">
      {/* 过程总题 */}
      <div className="rounded-2xl bg-slate-200 text-void p-4 sm:p-5 shadow-lg">
        <p className="text-xs font-bold opacity-50 tracking-widest uppercase">
          PROCESS · 对标最早航空航天页
        </p>
        <h3 className="mt-1 font-display text-2xl sm:text-3xl font-bold leading-tight">
          {data.title}
        </h3>
        <p className="mt-2 text-sm sm:text-base font-medium opacity-80 leading-relaxed">
          {data.subtitle}
        </p>
        <p className="mt-3 text-xs sm:text-sm font-bold opacity-60">
          结构固定：阶段路线 → 对比色块 → 趋势曲线 →（页内）硬指标 / 里程碑
        </p>
      </div>

      {/* A · 双路线阶段 — 对标 TechRoadmap */}
      <div>
        <SubHead n="A" title="中美阶段路线" hint="全过程主轴 · 四段演进" />
      ...
      {/* B · 对比要点大色块 — 对标最早页 CompareChip */}
      <div>
        <SubHead n="B" title="关键维度对照" hint="博物馆色块 · 中美各一句" />
            ...
            >
              ...
              <div className="grid grid-cols-1 divide-y divide-slate-800">
                <div className="px-4 py-3 bg-cyan-500/15">
                  <p className="text-xs font-bold text-cyan-300 mb-1 tracking-wide">美国</p>
                  <p className="text-sm sm:text-base font-semibold text-white leading-snug">
                    {c.left}
                  </p>
                </div>
                <div className="px-4 py-3 bg-amber-500/15">
                  <p className="text-xs font-bold text-amber-300 mb-1 tracking-wide">中国</p>
                  <p className="text-sm sm:text-base font-semibold text-white leading-snug">
                    {c.right}
                  </p>
                </div>
      ...
      {/* C · 趋势曲线 — 对标 LaunchChart */}
      <div>
        <SubHead n="C" title="对照曲线" hint="时间序列 · 同一坐标系" />
          ...
          <div className="h-[240px] sm:h-[300px] w-full">
            <ResponsiveContainer width="100%" height="100%">
              <AreaChart data={data.trend} margin={{ top: 8, right: 8, left: -12, bottom: 0 }}>
                <defs>
                  <linearGradient id={`procCyan-${data.domain}`} x1="0" y1="0" x2="0" y2="1">
                    <stop offset="0%" stopColor="#22d3ee" stopOpacity={0.4} />
                    <stop offset="100%" stopColor="#22d3ee" stopOpacity={0} />
                  </linearGradient>
                  <linearGradient id={`procAmber-${data.domain}`} x1="0" y1="0" x2="0" y2="1">
                ...
                <Area
                  ...
                  name="中国"
                  stroke="#f59e0b"
                  fill={`url(#procAmber-${data.domain})`}
                  strokeWidth={2.5}
                  dot={{ r: 3, fill: '#f59e0b' }}
                />
              </AreaChart>
...
function SubHead({ n, title, hint }: { n: string; title: string; hint: string }) {
  return (
    <div className="flex flex-wrap items-end justify-between gap-2 mb-3">
      <div className="flex items-end gap-2.5">
        <span className="font-mono-num text-2xl font-bold text-white/25 leading-none">{n}</span>
        <h4 className="font-display text-lg sm:text-xl font-bold text-white leading-none pb-0.5">
          {title}
        </h4>
      </div>
      <p className="text-xs sm:text-sm text-slate-500 font-medium">{hint}</p>
    </div>
  )
}
...
function RoadColumn({
  ...
  const border = tone === 'cyan' ? 'border-cyan-400/35' : 'border-amber-400/35'

  return (
    <div
      className={`rounded-2xl border ${border} overflow-hidden shadow-xl bg-[#070f1a]`}
    >
              ...
              <ul className="mt-1.5 space-y-1">
                {s.items.map((item) => (
                  <li key={item} className="text-sm text-slate-300 flex gap-2 leading-snug">
                    <span className={tone === 'cyan' ? 'text-cyan-400' : 'text-amber-400'}>
                      •
                    </span>
                    {item}
                  </li>
